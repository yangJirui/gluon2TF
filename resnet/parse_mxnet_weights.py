...
def read_mxnet_weights(path):
    ...
    name_tensor_dict = nd.load(path)

    name_array_dict = {}
    for name in sorted(name_tensor_dict.keys()):
        tensor = name_tensor_dict[name]
...
def read_resnet_50_v1d():
    ...
    path = download_mxnet_weights('resnet%d_v%dd'%(50, 1),
                            tag=True)

    read_mxnet_weights(path)
