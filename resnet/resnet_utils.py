...
def bottleneck_v1b(input_x, input_channel, stride=1, avg_down=True):
    '''
    for bottleneck_v1b: reduce spatial dim in conv_3x3 with stride 2.
    '''
    net = slim.conv2d(input_x, num_outpus=input_channel, kernel_size=[1, 1], stride=1,
                      padding="SAME", biases_initializer=None, data_format=DATA_FORMAT)
    if stride == 2:
        net = slim.conv2d(net, num_outputs=input_channel, kernel_size=[3, 3], stride=2,
                          padding="SAME", biases_initializer=None, data_format=DATA_FORMAT)
        if avg_down:  # design for resnet_v1d
            ...
            shortcut = slim.conv2d(shortcut, num_outputs=input_channel*4, kernel_size=[1, 1],
                                   stride=1, padding="SAME", biases_initializer=None, data_format=DATA_FORMAT)
            # shortcut should have batch norm.
        else:
            shortcut = slim.conv2d(input_x, num_outputs=input_channel * 4, kernel_size=[1, 1],
    ...
    net = slim.conv2d(net, num_outpus=input_channel*4, kernel_size=[1, 1], stride=1,
                      padding="SAME", biases_initializer=None, data_format=DATA_FORMAT,
                      activation_fn=None)
...
def make_block(net, input_channel, bottleneck_nums, scope, avg_down=True, spatial_downsample=False):
    with tf.variable_scope(scope):
        first_stride = 2 if spatial_downsample else 1
        with tf.variable_scope("bottleneck_0"):
            net = bottleneck_v1b(input_x=net, input_channel=input_channel,
                                 stride=first_stride, avg_down=avg_down)
        for i in range(1, bottleneck_nums):
            with tf.variable_scope("bottleneck_%d" % i):
                net = bottleneck_v1b(input_x=net, input_channel=input_channel,
                                     stride=1, avg_down=avg_down)
        return net
    ...
    with tf.variable_scope(scope):
        ...
        for i in range(2, len(bottleneck_nums)+2):
            ...
            with slim.arg_scope(resnet_arg_scope(is_training=not freeze[i-2])):
                net = make_block(net=net, input_channel=base_channels[i-2],
